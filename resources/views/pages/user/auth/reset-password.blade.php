...
@section('title')
    @lang('user.pages.reset_password.page_title')
@stop
...
@section('content')
    ...
    <div class="contents">
        <form action="{!! action('User\PasswordController@postResetPassword') !!}" method="post">
            {!! csrf_field() !!}
        <section>
            <h2 class="title-m cent">@lang('user.pages.reset_password.contents_input')</h2>
            <p class="cent em sp-btm">@lang('user.pages.reset_password.please_enter_new_password')</p>
            <table class="form-a">
                <tr>
                    <th>@lang('user.pages.reset_password.email')</th>
                    <td><input type="email" name="email" placeholder="@lang('user.pages.reset_password.email')"
                               class=" @if ($errors->has('email')) alert_outline @endif">
                        @if ($errors->has('email'))<p class="alert"> {!! $errors->first('email') !!}</p> @endif</td>
                </tr>
                <tr>
                    <th>@lang('user.pages.reset_password.password')</th>
                    <td><input type="password" name="password" placeholder="@lang('user.pages.reset_password.password')"
                               class=" @if ($errors->has('password')) alert_outline @endif">
                        @if ($errors->has('password'))<p
                                class="alert"> {!! $errors->first('password') !!}</p> @endif</td>
                </tr>
                <tr>
                    <th>@lang('user.pages.reset_password.password_confirm')</th>
            ...
            <p><a  href="#" onclick="$(this).parents('form:first').submit(); return false;" class="btn-o">@lang('user.pages.reset_password.submit')</a></p>
        </section>
        </form>
    </div>
@stop
