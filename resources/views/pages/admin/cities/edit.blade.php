            ...
            <div class="box box-primary">
                ...
                <div class="box-body">
                    ...
                    <div class="form-group @if ($errors->has('name_ja')) has-error @endif">
                        <label for="name_ja">@lang('admin.pages.cities.columns.name_ja')</label>
                        <input type="text" class="form-control" id="name_ja" name="name_ja" value="{{ old('name_ja') ? old('name_ja') : $city->name_ja }}">
                    </div>
                        ...
                        <select id="country_code" class="selectpicker"  name="country_code" data-live-search="true" title="Please select a country ...">
                            @foreach ($countries as $country)
                                <option value="{{ $country->code }}" {{ old('country_code') ? 'selected' : '' }}
                                        {{ $country->code==$city->country_code ? 'selected' : '' }}>{{ $country->name_en . '-' .$country->name_ja}}</option>
                            @endforeach
